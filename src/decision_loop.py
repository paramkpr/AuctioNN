...
class DecisionLoop:
    def __init__(
        self,
        predictor: torch.jit.ScriptModule | Callable[[torch.Tensor, torch.Tensor], torch.Tensor],
    ...
    def process(self, imp: Impression) -> None:
        ...
        if len(pconv_batch) != len(carriers):
            ...
            print(f"Error: Mismatch in prediction batch size ({len(pconv_batch)}) and eligible campaigns ({len(carriers)})")
            return
        ...
        if self.logger:
            log_data: Dict[str, Any] = {
                ...
                "utility": (expected_value - market_price) if won else 0.0, # Utility is 0 if not won
