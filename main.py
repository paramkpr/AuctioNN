...
from src.models.network import (
    ImpressionConversionNetwork,
)  # Used for fallback model loading
...
def fit_preprocessors(
    ...
    try:
        ...
        click.echo(
            f"Splitting data (Test={test_split_ratio * 100:.0f}%, Val={val_split_ratio * 100:.0f}% of remainder)..."
        )
        ...
        if test_split_ratio > 0:
            ...
            if not can_stratify_full:
                click.echo(
                    "Warning: Cannot stratify initial test split (only one class in full dataset)."
                )
            ...
            targets_train_val_np = (
                train_val_df[target_column].to_numpy()
        ...
        if val_split_ratio > 0 and not train_val_df.empty:
            ...
            relative_val_ratio = (
                val_split_ratio / current_train_val_ratio
                if current_train_val_ratio > 0
                else val_split_ratio
            )

            if relative_val_ratio >= 1.0:
                click.echo(
                    "Warning: Validation split ratio results in empty training set. Using all non-test data for training."
                )
                ...
                if not can_stratify_val:
                    click.echo(
                        "Warning: Cannot stratify validation split (only one class in train_val data). Performing non-stratified split."
                    )
                ...
                try:
                    ...
                    click.echo(
                        f"Warning: Stratified validation split failed ({e}). Performing non-stratified split."
                    )
            ...
            click.echo(
                "Skipping validation split (ratio is 0 or no data left). Using all non-test data for training."
            )
            train_df = train_val_df

        if train_df.empty:
            raise ValueError(
                "Training data split resulted in an empty DataFrame. Check split ratios."
            )

        click.echo(
            f"Fitting preprocessors using training split (shape: {train_df.shape})..."
        )
        ...
        click.echo(
            f"ERROR: Cleaned data file not found at {cleaned_data_file}", err=True
        )
...
@click.option(
    ...
    default=0.15,
    type=click.FloatRange(0.0, 0.9),
    help="Proportion of non-test for validation.",
)
@click.option(
    "--target-column",
    default="conversion_flag",
    help="Target column for stratification.",
)
@click.option("--random-state", default=42, type=int, help="Random seed for splits.")
# Other training hyperparameters
@click.option(
    "--learning-rate", default=1e-3, type=float, help="Optimizer learning rate."
)
@click.option(
    "--embedding-dim",
    default=32,
    type=int,
    help="Dimension for categorical embeddings.",
)
def train(
    epochs,
    batch_size,
    ...
    try:
        ...
        click.echo(
            f"Data splits created: Train={train_df.shape}, Val={val_df.shape}, Test={test_df.shape}"
        )

        # --- Apply Preprocessing to Splits ---
        os.makedirs(processed_data_dir, exist_ok=True)
        # Check if files already exist (using one file as a proxy)
        train_tgt_exists = os.path.exists(
            os.path.join(processed_data_dir, "train_target_data.npy")
        )
        val_tgt_exists = os.path.exists(
            os.path.join(processed_data_dir, "val_target_data.npy")
        )
        test_tgt_exists = os.path.exists(
            os.path.join(processed_data_dir, "test_target_data.npy")
        )

        # Determine if reprocessing is needed for any split
        needs_reprocessing = (
            force_reprocess
            or not train_tgt_exists
            or (not val_df.empty and not val_tgt_exists)
            or (not test_df.empty and not test_tgt_exists)
        )
        ...
        if needs_reprocessing:
            ...
            click.echo(
                f"Found existing processed splits in {processed_data_dir}, skipping reprocessing."
            )
...
@click.option("--batch-size", default=1024, type=int, help="Batch size for evaluation.")
@click.option(
    "--threshold",
    default=0.5,
...
def simulate(
    ...
    try:
        ...
        model = load_model(model, torch_device)
