...
def count_pos_neg(
    processed_data_dir: str, split: str = "train", target_col: str = "conversion_flag"
) -> tuple[int, int]:
    """
...
def infer_feature_counts(
    processed_data_dir: str, split: str = "train"
) -> tuple[int, int]:
    """
    Inspects the schema once and returns (n_categorical, n_numerical).
    """
    schema = ds.dataset(
        os.path.join(processed_data_dir, split), format="parquet"
    ).schema
...
def run_training(
    ...
    try:
        ...
        category_sizes = joblib.load(
            os.path.join(preprocessor_dir, "category_sizes.joblib")
        )
    ...
    train_dataset = ParquetAuctionDataset(train_dir, batch_rows=batch_size)
    val_dataset = ParquetAuctionDataset(val_dir, batch_rows=batch_size)
    ...
    train_loader = DataLoader(
        ...
        pin_memory=True,
        # collate_fn=lambda x: x[0],  # Collate function for IterableDataset
    )
    ...
    criterion = nn.BCEWithLogitsLoss(
        pos_weight=torch.tensor(pos_weight_value, device=resolved_device)
    )
    ...
    train_rows = ds.dataset(train_dir, format="parquet").count_rows()
    ...
    for epoch in range(epochs):
        ...
        if val_loader:  # Only run validation if loader exists
            ...
            print(
                f"Epoch {epoch + 1}/{epochs} => Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f} | Val Acc: {accuracy:.4f}"
            )
            ...
            if avg_val_loss < best_val_loss:
                ...
                print(
                    f"  -> Saved best model to {save_model_path} (Val Loss: {best_val_loss:.4f})"
                )
        else:
            # No validation, just print train loss
            print(
                f"Epoch {epoch + 1}/{epochs} => Train Loss: {avg_train_loss:.4f} | (No validation)"
            )
